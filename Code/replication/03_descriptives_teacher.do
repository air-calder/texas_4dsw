...
foreach v in teachid syear district campus post_adoption ever4DSW is_incumbent is_entrant female certified exper totalpay fte class_size class_frpl_share class_nonwhite_share class_prior_ach stay_school_t1 stay_district_t1 switch_district_t1 exit_tx_public_t1 incoming_from_tx incoming_first_time incoming_alt_path incoming_experience incoming_adv_degree incoming_no_degree {
...
preserve
sort syear teachid
by syear teachid: gen __tag_teacher = (_n == 1)
...
sort syear
export delimited using "code/4DSW student teacher analysis/replication/output/descriptives/sample_counts_by_year.csv", replace
restore
...
export delimited using "code/4DSW student teacher analysis/replication/output/descriptives/adoption_timing_distribution.csv", replace
restore

* 3) Pre-period balance means by ever-treated status.
preserve
keep if syear <= 2019
collapse (mean) female certified exper totalpay fte class_size class_frpl_share class_nonwhite_share class_prior_ach stay_school_t1 stay_district_t1 switch_district_t1 exit_tx_public_t1, by(ever4DSW)
export delimited using "code/4DSW student teacher analysis/replication/output/descriptives/preperiod_balance_means.csv", replace
restore
...
save "code/4DSW student teacher analysis/replication/output/descriptives/retention_trends_by_group.dta", replace
export delimited using "code/4DSW student teacher analysis/replication/output/descriptives/retention_trends_by_group.csv", replace
restore
...
save "code/4DSW student teacher analysis/replication/output/descriptives/entrant_trends_by_group.dta", replace
export delimited using "code/4DSW student teacher analysis/replication/output/descriptives/entrant_trends_by_group.csv", replace
restore
...
foreach y in stay_school_t1 stay_district_t1 exit_tx_public_t1 {
    ...
    graph export "code/4DSW student teacher analysis/replication/output/figures/trend_`y'.png", replace width(1400) height(900)
}
