...
foreach v in teachid syear district firstyear ever4DSW post_adoption is_incumbent stay_school_t1 stay_district_t1 switch_district_t1 exit_tx_public_t1 {
...
sort teachid syear
by teachid: egen g_first_treat = min(cond(post_adoption == 1, syear, .))
...
foreach y in stay_school_t1 stay_district_t1 switch_district_t1 exit_tx_public_t1 {
    ...
    forvalues a = 0/1 {
        capture noisily csdid `y' if is_incumbent == 1 & !missing(`y'), ivar(teachid) time(syear) gvar(g_first_treat) notyet cluster(district) anticipation(`a')
        if _rc {
