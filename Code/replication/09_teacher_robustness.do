...
foreach v in teachid syear district campus is_incumbent post_adoption rural hybrid_calendar stay_school_t1 stay_district_t1 {
...
foreach sp of local specs {
    ...
    foreach y in stay_school_t1 stay_district_t1 {
        if "`sp'" == "teacher_fe" {
            noisily areg `y' post_adoption i.syear if `if_cond' & !missing(`y'), absorb(teachid) vce(cluster district)
        }
        else {
...
use "`rb'", clear
sort outcome spec
export delimited using "code/4DSW student teacher analysis/replication/output/tables/teacher_robustness.csv", replace
