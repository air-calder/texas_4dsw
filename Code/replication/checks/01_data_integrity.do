...
local required "teachid campus district syear post_adoption ever4DSW firstyear event_time is_incumbent is_entrant stay_school_t1 stay_district_t1 switch_district_t1 exit_tx_public_t1 incoming_from_tx incoming_first_time incoming_alt_path incoming_experience incoming_adv_degree incoming_no_degree"
...
capture noisily isid teachid syear
if _rc {
    di as error "Teacher-year key teachid x syear is not unique"
    exit 459
}
...
postclose `varpost'
use "`varcheck'", clear
export delimited using "code/4DSW student teacher analysis/replication/output/checks/required_variable_check.csv", replace
...
export delimited using "code/4DSW student teacher analysis/replication/output/checks/key_check.csv", replace
restore
...
// use "`miss'", clear
// sort -pct_missing
// export delimited using "code/4DSW student teacher analysis/replication/output/checks/missingness_summary.csv", replace
...
clear
set obs 1
gen fail_stayschl_impl_staydist = `c_stay'
gen fail_obsequals_stayplusswitch = `c_move_id'
gen fail_obsplus_exitequals_one = `c_exit_id'
export delimited using "code/4DSW student teacher analysis/replication/output/checks/transition_identity_checks.csv", replace
restore
...
sort syear
export delimited using "code/4DSW student teacher analysis/replication/output/checks/year_level_counts.csv", replace
